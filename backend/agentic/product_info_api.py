...
Unified API endpoint for Product Info queries with intelligent routing
...
def query_endpoint():
    """
    Main query endpoint for Product Info.
    ...
    try:
        ...
        session_id = data.get('session_id') or session.get('product_info_session_id')
        if not session_id:
            session_id = f"pi_{session.get('user_id', 'anon')}_{id(request)}"
            session['product_info_session_id'] = session_id
        
        logger.info(f"[PRODUCT_INFO_API] Query from session {session_id}: {query[:100]}...")
        
        # Run the orchestrated query
        result = run_product_info_query(
            query=query,
        ...
        logger.error(f"[PRODUCT_INFO_API] Error: {e}", exc_info=True)
        return jsonify({
            'success': False,
            'error': str(e),
...
@product_info_bp.route('/session', methods=['GET'])
@login_required
def get_session_info():
    """Get current session summary."""
    try:
        session_id = session.get('product_info_session_id')
        if not session_id:
            return jsonify({
                'success': True,
                'session': None,
        ...
        logger.error(f"[PRODUCT_INFO_API] Session info error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@product_info_bp.route('/session', methods=['DELETE'])
@login_required
def clear_session_endpoint():
    ...
    try:
        ...
        if session_id:
            ...
            session.pop('product_info_session_id', None)
        ...
        logger.error(f"[PRODUCT_INFO_API] Clear session error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@product_info_bp.route('/cleanup', methods=['POST'])
@login_required
def cleanup_sessions():
    ...
    try:
        ...
        logger.error(f"[PRODUCT_INFO_API] Cleanup error: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
@product_info_bp.route('/health', methods=['GET'])
def health_check():
